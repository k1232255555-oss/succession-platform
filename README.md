...
メール通知を使う場合は以下を設定します。未設定でもスカウト、メッセージ、決済処理は止まりません。通知ログは `SKIPPED` として保存されます。
...
3. スカウト文、希望面談日時、β期間中無料の確認を入力して送信
4. `/scouts` で状態を更新
...
スカウト作成と状態更新は監査ログに記録されます。`feeAcknowledged` は既存互換のため列名を維持し、初期リリースではβ無料確認として扱います。

## メッセージ

`/messages` でスカウトに紐づくメッセージスレッドを確認できます。
...
6. 必要なら `NOTIFICATION_REPLY_TO` を追加
7. `NOTIFICATION_EMAILS_ENABLED` を `true` に変更
8. Redeploy
...
初期リリースは **β期間中無料** です。`STRIPE_BILLING_ENABLED=false` の間は、AIマッチング、スカウト、メッセージを無料で利用できます。Stripeコード、Webhook、Billing画面は残しているため、将来 `STRIPE_BILLING_ENABLED=true` とStripe環境変数を設定すれば課金導線を有効化できます。

運用メモ:

- 初期は候補者100〜300人、企業20〜50社程度までは無料運用を想定します。
- この想定規模は画面には表示しません。
- Dashboardの数字は必ずDB集計値のみを表示します。
- 架空の参加企業数、候補者数、マッチング数は表示しません。
...
支払い失敗時は会社の `billingStatus` を `PAST_DUE` に更新し、新規スカウト送信を止めます。既存データの閲覧は止めません。メール通知が有効な場合は `OWNER / ADMIN` に決済失敗通知を送信します。
