...
未来へ、事業をつなぐ。Succession Club は、日本の黒字廃業を減らし、技術・雇用・地域経済を未来へつなぐための事業承継プラットフォームです。

M&A、親族承継、従業員承継を否定せず、承継に向き合う人にとってのもう一つの選択肢として設計します。AIは中心価値ではなく、承継検討を補助する参考機能の一つです。
...
- 企業オーナー、後継者、従業員、家族それぞれにとって最善かを判断基準にします
- 短期売上より長期的な信頼を優先します
- 誇張表現、根拠のない人数表示、利用実績の水増しは禁止です
- 初期企業は Founding Members として扱い、最初の50社はStandard相当を永久無料で運用する方針です
...
初期リリースは **β期間中無料** です。`STRIPE_BILLING_ENABLED=false` の間は、参考分析、対話申請、メッセージを無料で利用できます。Stripeコード、Webhook、Billing画面は残しているため、将来 `STRIPE_BILLING_ENABLED=true` とStripe環境変数を設定すれば課金導線を有効化できます。
