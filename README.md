...
未来へ、事業をつなぐ。Succession Club は、後継者不足に悩む事業者と、事業を引き継いで挑戦したい人をつなぐ仕組みを検証する、招待制クローズドβです。現在は事業者側からの候補者閲覧・対話申請を中心に検証しています。
...
- 第1期の初期参加事業者を審査制で最大10社まで募集します
- 初期参加事業者には、現在提供中の基本機能をβ終了後も無料で提供する方針です。対象範囲や将来の有料機能は事前に明示します
...
通常のメール通知を使う場合は以下を設定します。未設定でも対話申請、メッセージ、決済処理は止まりません。通知ログは `SKIPPED` として保存されます。
...
`/contact` の問い合わせフォームは、通常通知の `NOTIFICATION_EMAILS_ENABLED` とは独立して動作します。本番で問い合わせを受け付けるには、Vercel Productionへ以下の3項目を必ず設定してください。
...
3項目が揃った場合のみ運営宛への送信を成功扱いにします。未設定またはResend送信失敗時は受付完了と表示せず、個人情報をサーバーログへ出力しません。`SUPPORT_EMAIL` を設定した場合のみ、問い合わせページに公開連絡先として表示します。
...
候補者は本人同意と審査を確認したうえで管理画面から登録します。架空候補者やサンプル候補者を本番へ投入するseedコマンドは提供しません。
...
Succession Club は、事業を残したい人と事業を引き継ぎたい人をつなぐ仕組みの実現を目指しています。現在のクローズドβでは、事業者側からの候補者閲覧・対話申請を中心に検証します。

承継ブリーフは、マッチング前に事業者の状況や承継の詰まりポイントを整理し、対話や実名案件化を進めやすくするための補助機能です。匿名・構造化された承継論点データは、サービス改善と安全な論点整理のために利用します。

## 残したい事業と承継プロジェクト

将来の双方向化に向けた土台として、事業者が「何を引き継いでほしいのか」を匿名・限定公開前提で整理できる `BusinessOpportunity` を追加しています。
...
クローズドβでは、初期から取り組みに参加してくださる事業者を最大10社まで募集します。第1期の初期参加事業者には、現在提供している基本機能をβ終了後も無料で提供する方針です。対象範囲や将来追加する有料機能は事前に明示します。共同運営者の募集ではありません。
...
初期リリースは **招待制クローズドβ** です。`STRIPE_BILLING_ENABLED=false` を維持し、第1期の初期参加事業者には現在提供中の基本機能を無料で提供します。Stripeコード、Webhook、Billing画面は残しますが、課金開始は料金・解約・返金方針の再審査後に明示的に承認します。

運用メモ:

- βでは大量集客ではなく、少数企業での課題検証を優先します。
- 初期参加枠は先着ではなく、運営が確認できる審査制で最大10社までとします。
- 初期参加事業者の無料対象範囲と、将来追加する有料機能は事前に明示します。
...
追加開発は、実際の利用者要望と運用上の必要性を確認してから着手します。

- 候補者側ログインや双方向オファーは、クローズドβの検証結果を確認してから判断
- 有料プランは、提供価値、解約タイミング、返金、運用負荷を再審査してから判断

## クローズドβの初期運用方針

- クローズドβは無料で運用し、Stripe課金は有効化しません。
- 審査制を維持し、本人同意のない候補者情報・事業情報は登録または公開しません。
- 広告・アフィリエイトモデルは、中立性と信頼への影響が大きいため現時点では採用しません。
- 初期10社をSNS投稿だけで集める前提にはしません。
- 税理士、商工会、事業承継支援機関、地域金融機関など10組織へ個別に説明し、各組織から1社の紹介を依頼します。
- 詐欺、無断掲載、情報収集目的の利用を防ぐため、本人同意、審査、権限管理、監査ログを優先します。
